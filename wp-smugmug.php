...
@define('WP_SMUGMUG_VERSION', '2.0.8');
...
@define('WP_SMUGMUG_CSS_IE', 'ul.thumbwrap li {
	width: 163px;
	w\idth: 161px;
	display: inline;
}
		...
		function wp_smugmug_shortcode($atts, $content = null)
		{
			...
			if ( !empty($url) ) {
				...
				if ( is_wp_error($rss) || empty($rss) || count($rss->get_items()) == 0 ) {
					...
					$output .= '<p><a href="http://www.smugmug.com/gallery/' . $album_id . '"' . $target . '>' . __('View photos at SmugMug', $this->wp_smugmug_domain)  . '</a></p>';
					...
					foreach($items as $item) {
						...
						$item_title = htmlspecialchars(strip_tags($item->get_title()));	
						$rel = '';
						...
						switch ( $link ) {
							...
							case 'lightbox':
								$the_link = $photo_url;
								$rel = ' rel="lightbox[wp-smugmug-' . $post->ID . ']" class="lightbox fancybox"';
								break;
						...
						if ( $captions ) {
							...
							$output .= wptexturize($item->get_title());
							$output .= '</span>';
						}
						$output .= '</a>';
						$output .= '</div></li>';
					...
					if ( $smugmug )
						$output .= '<p style="text-align: center;"><a href="' . $rss->get_link() . '"' . $target . '>' . __('View photos at SmugMug', $this->wp_smugmug_domain) . '</a></p>';
